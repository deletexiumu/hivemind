...
<objective>
Create all phases necessary to close gaps identified by `/dw:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/dw:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns dw-planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/dw:audit-milestone` first.
```
...
**Also available:**
- `/dw:execute-phase {N}` — if plans already exist
...
`/dw:audit-milestone` — re-audit to verify gaps closed
`/dw:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/dw:plan-phase` next
</success_criteria>
