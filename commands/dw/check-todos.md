...
Todos are captured during work sessions with /dw:add-todo.

---

Would you like to:

1. Continue with current phase (/dw:progress)
2. Add a todo now (/dw:add-todo)
```
...
- `/dw:check-todos` → show all
- `/dw:check-todos api` → filter to area:api only
</step>
...
- `/dw:check-todos [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /dw:add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/dw:add-phase [description from todo]`
...
- Don't create plans from this command — route to /dw:plan-phase or /dw:add-phase
</anti_patterns>
