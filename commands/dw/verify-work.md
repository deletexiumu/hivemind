...
Output: {phase}-UAT.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for /dw:execute-phase
</objective>

<execution_context>
@~/.claude/hivemind/workflows/verify-work.md
@~/.claude/hivemind/templates/UAT.md
</execution_context>
...
8. If issues found:
   ...
   - Spawn dw-planner in --gaps mode to create fix plans
   - Spawn dw-plan-checker to verify fix plans
   - Iterate planner ↔ checker until plans pass (max 3)
   - Present ready status with `/clear` then `/dw:execute-phase`
</process>
...
- /dw:execute-phase {Z+1} — skip to execution (if already planned)
...
**Also available:**
- /dw:complete-milestone — skip audit, archive directly
...
- /dw:plan-phase {Z} --gaps — regenerate fix plans
...
**Options:**
- /dw:plan-phase {Z} --gaps — retry fix planning with guidance
- /dw:discuss-phase {Z} — gather more context before replanning
...
- [ ] If issues: dw-planner creates fix plans from diagnosed gaps
- [ ] If issues: dw-plan-checker verifies fix plans (max 3 iterations)
- [ ] Ready for `/dw:execute-phase` when complete
</success_criteria>
