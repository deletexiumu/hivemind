...
Plans:
- [x] 01-01-PLAN.md — 创建目录结构 + 编写术语表（89 条目）✓
- [x] 01-02-PLAN.md — 编写命名规范 + 提示规范文档 ✓
...
Plans:
- [x] 03-01-PLAN.md — 平台约束索引 + Hive 平台约束 (PLATFORM-01) ✓
- [x] 03-02-PLAN.md — dbt-hive 能力边界 (PLATFORM-02) + 增量策略 (PLATFORM-03) ✓
...
1. 模型评审提示 (`prompts/scenarios/review-existing-model/prompt.md`)
   ...
   - 评审维度（命名、分层、粒度、字段、dbt 配置）
   - 输出结构（问题列表 + 修复建议）
   - 两段式交互（Stage 1 问题概览 + Stage 2 详细修复）
...
5. 输出模板 (`prompts/scenarios/review-existing-model/output-template.md`)
   - Stage 1：评审总览 + 检查清单（聚合视图）+ 问题摘要
   - Stage 2：详细修复建议（按问题逐个展示）

6. 案例库 (`prompts/scenarios/review-existing-model/examples/`)
   - 高质量模型案例（通过）
   - 命名问题案例（有条件通过）
   - 多问题严重缺陷案例（不通过）
...
4. 在 3 个测试用例（从"非常好"到"有多个问题"）上的评审结果可信度 > 85%
5. 提示 token < 2000
