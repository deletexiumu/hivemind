...
Purpose: 这是 Phase 4 的核心交付物，用户通过此提示输入业务事件/粒度，获得完整的星型模型设计（事实表、维度表、DDL、schema.yml、dbt SQL 模板）。
...
<task type="auto">
  ...
  <action>
    ...
    **CONTEXT 块 (~100 tokens):**
    - 占位符说明：`{运行时注入的上下文：来自 includes 声明的 *-core 文件}`
    - 标注：Phase 4 由执行者手动组装/注入，Phase 8 工具化自动组合

    **TASK 块 (~300 tokens):**
    - 输入格式说明（混合模式：YAML 模板 + 自由文本）
    - 必填最小集：业务事件 + 粒度
    - 可选信息：指标需求、维度清单、字段清单、数据源信息
    - 缺失信息处理策略：先推断，列出假设清单，请用户确认
    - 支持多事件统一建模
  ...
  <done>
    主提示文件 prompt.md 创建完成，包含 4-block 结构、两段式交互说明、决策指导原则、内嵌简单案例，token 数 < 2000
  </done>
</task>
...
<task type="auto">
  ...
  <done>
    输出模板文件 output-template.md 创建完成，包含 5 个核心部分 + 输出交付契约 + 自检清单，token 数 < 1000
  </done>
</task>
...
3. output-template.md 文件存在且包含：
   ...
   - 输出交付契约
   - 自检清单
4. prompt.md token 数 < 2000
5. output-template.md token 数 < 1000
...
- 两段式交互设计完整
- 输出模板覆盖 5 个核心部分
- Token 预算合规（prompt.md < 2000, output-template.md < 1000）
