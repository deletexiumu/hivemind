...
<task type="auto">
  ...
  <action>
    ...
    **fact-table-types-core.md** (~600-800 tokens):
    ...
    - 来源标注：`Source: fact-table-types.md | Updated: {date}`
    ...
    **scd-strategies-core.md** (~800-1000 tokens):
    ...
    - 来源标注：`Source: scd-strategies.md | Updated: {date}`

    格式要求：
    - 使用表格、决策树、检查清单
    - 不写长篇解释段落
    - 每个文件开头添加 frontmatter：type: context-core, source: {原文件路径}
    - 每个文件末尾添加 Source 标注行，格式：`Source: {原文件名}.md | Updated: YYYY-MM-DD`
  </action>
  <verify>
    统计每个文件字数（中文 1 token ≈ 1.5 字，目标字数 = token 上限 × 1.5）：
    ```bash
    wc -w .claude/data-warehouse/context/methodology/dimensional-modeling-core.md
    # 目标：字数 < 1500（对应 1000 tokens）

    wc -w .claude/data-warehouse/context/methodology/fact-table-types-core.md
    # 目标：字数 < 1200（对应 800 tokens）

    wc -w .claude/data-warehouse/context/methodology/scd-strategies-core.md
    # 目标：字数 < 1500（对应 1000 tokens）
    ```
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    **layering-system-core.md** (~600-800 tokens):
    ...
    - 跨层引用规则（允许/禁止矩阵）
    - 维度表跨层引用说明
    - 来源标注：`Source: layering-system.md | Updated: {date}`
    ...
    **hive-constraints-core.md** (~800-1000 tokens):
    ...
    - 动态分区配置模板
    - 禁止 SELECT * 规则
    - 来源标注：`Source: hive-constraints.md | Updated: {date}`
    ...
    **dbt-hive-limitations-core.md** (~600-800 tokens):
    ...
    - 增量模型标准配置模板
    - 来源标注：`Source: dbt-hive-limitations.md | Updated: {date}`
  ...
  <verify>
    统计每个文件字数：
    ```bash
    wc -w .claude/data-warehouse/context/layers/layering-system-core.md
    # 目标：字数 < 1200（对应 800 tokens）

    wc -w .claude/data-warehouse/context/platform/hive-constraints-core.md
    # 目标：字数 < 1500（对应 1000 tokens）

    wc -w .claude/data-warehouse/context/platform/dbt-hive-limitations-core.md
    # 目标：字数 < 1200（对应 800 tokens）
    ```
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    **naming-core.md** (~600-800 tokens):
    ...
    - 来源标注：`Source: naming.md | Updated: {date}`
  ...
  <verify>
    ...
    # 目标：总字数 < 9000（对应 6000 tokens）

    # 验证 Source 标注存在
    grep -E 'Source:.*\.md' .claude/data-warehouse/context/methodology/*-core.md
    grep -E 'Source:.*\.md' .claude/data-warehouse/context/layers/*-core.md
    grep -E 'Source:.*\.md' .claude/data-warehouse/context/platform/*-core.md
    grep -E 'Source:.*\.md' .claude/data-warehouse/docs/naming-core.md
    ```
  </verify>
  <done>
...
4. 单文件 token 数 < 2000（字数 < 3000）
5. 7 个文件合计 token 数 < 6000（字数 < 9000）
