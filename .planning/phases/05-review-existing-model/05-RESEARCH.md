...
### Pattern 3: 分级评分制 (Weighted Severity Scoring)

**What:** 从 100 分开始，按问题严重级别扣分
**When to use:** 量化评审结果，提供可比较的质量指标
...
| 级别 | 扣分 | 含义 |
|------|------|------|
| P0 严重 | -25 | 数据质量问题，必须修复 |
| P1 高优 | -10 | 设计缺陷，强烈建议修复 |
| P2 中优 | -3 | 风险提示，建议优化 |
| P3 低优 | -1 | 代码风格，可选修复 |
...
| **元信息** | 分层声明、粒度声明 | 主动追问，待补充后再评审 |

**可选增强：**
| 字段 | 说明 | 影响 |
|------|------|------|
| dbt 配置 (schema.yml) | dbt tests/meta/description | 可进行 dbt 配置检查 |
| DDL | CREATE TABLE 语句 | 可进行存储格式检查 |
...
| 输入 | 可评审维度 |
|------|-----------|
| 仅 SQL | 命名、分层引用（从 ref/source 推断） |
| SQL + 元信息 | 命名、分层引用、粒度主键 |
| SQL + 元信息 + schema.yml | 全部五个维度 |
...
| 5 | P2 | dbt 配置 | 关键字段缺少 tests | `order_id` |
| 6 | P2 | 可加性 | 度量可加性未标注 | `quantity`, `amount` |
| 7 | P3 | 命名规范 | 命名缩写不一致 | `qty` vs `quantity` |
| 8 | P3 | dbt 配置 | 外键 test 缺失 | `customer_key` |
...
**What we know:** 用户决策"超过 3 处变更时拆分为多步骤"
**What's unclear:** 如何判断"处"？是代码行数还是逻辑修改点？
**Recommendation:**
- 以"逻辑修改点"为准（如：改表名是 1 处，改 5 个字段名是 5 处）
- Claude 自行判断，倾向于拆分以确保清晰

### 2. 跨文件修复的输出组织

**What we know:** 评审可能涉及 SQL + schema.yml 两个文件
**What's unclear:** Stage 2 如何组织多文件修复建议？
**Recommendation:**
- 按问题组织，每个问题包含所有相关文件的修复
- 使用 `### File: {path}` 格式（与 Phase 4 一致）

### 3. 部分通过的评审结论

**What we know:** P0=0 且分数>=70 为通过
**What's unclear:** 如果 P0=0 但分数=65，结论是什么？
**Recommendation:**
- "有条件通过"：无严重问题，但建议修复 P1/P2 以达到 70 分
- 在评审总览中明确说明
