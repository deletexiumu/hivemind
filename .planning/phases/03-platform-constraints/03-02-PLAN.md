...
5. **功能限制详解（使用统一模板）：**
   ...
   **规避方案:**
   ...
   2. 参考 [SCD 策略文档](../methodology/scd-strategies.md) 获取 SCD 实现模式
   3. **具体增量策略实现见 [增量策略文档](./incremental-strategies.md)**
   **链接:** [增量策略](./incremental-strategies.md) - 包含完整的 insert_overwrite 分区回刷代码示例
...
- 每个限制有替代方案
- P0 限制有代码示例
- DBT-HIVE-001 规避方案明确链接到 incremental-strategies.md
  </verify>
  <done>dbt-hive 限制文档完整，每个限制有明确替代方案，关键限制链接到增量策略文档</done>
</task>
...
10. **与 SCD 策略的关系：**
    - 增量策略用于事实表
    - SCD 策略用于维度表
    - 链接到 [SCD 策略文档](../methodology/scd-strategies.md) 获取维度表处理模式
...
- 运行命令示例
- 全量/增量决策矩阵
- lookback 配置章节引用 scd-strategies.md
- 分区内去重章节引用 scd-strategies.md
  </verify>
  <done>增量策略文档完整，包含 T+1 + lookback 模式的完整实现，关键章节链接到 SCD 策略</done>
</task>
...
3. 确认约束速查表完整：
   - 包含所有 HIVE-xxx 约束（至少 HIVE-001 ~ 020）
   - 包含所有 DBT-HIVE-xxx 约束（至少 DBT-HIVE-001 ~ 005）
   - 确认 P0 约束高亮/置顶
...
4. 验证所有文档链接正确可访问
  ...
  <verify>
验证约束速查表完整性：
- 约束速查表包含 >=8 个 P0 约束（HIVE-xxx 和 DBT-HIVE-xxx 合计）
- 手动测试 3 个主要链接可点击访问：
  1. hive-constraints.md
  2. dbt-hive-limitations.md
  3. incremental-strategies.md
- 索引页 status: stable
- 所有文档状态为 stable
  </verify>
  <done>Phase 03 平台约束库完成，所有文档状态为 stable，速查表包含 >=8 个 P0 约束</done>
</task>
...
7. **索引页更新：** 所有文档状态为 stable，速查表 >=8 个 P0
8. **文档链接：** key_links 声明的链接在文档中明确出现
9. **Token 预算：** dbt-hive <1500, 增量策略 <1800
</verification>
...
- [ ] 每个 dbt-hive 限制有替代方案
- [ ] DBT-HIVE-001 明确链接到 incremental-strategies.md
- [ ] 增量策略包含完整的 dbt 模型代码
- [ ] lookback 配置和分区内去重引用 scd-strategies.md
- [ ] 索引页所有文档状态为 stable
- [ ] 约束速查表包含 >=8 个 P0 约束
- [ ] 文档符合 token 预算限制
</success_criteria>
