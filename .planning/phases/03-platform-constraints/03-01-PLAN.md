...
must_haves:
  truths:
    - "用户能通过索引页快速定位所有平台约束"
    - "Hive 约束按 P0/P1/P2 风险等级组织"
    - "每个 Hive 约束包含原因、后果、规避方案、正/反示例"
    - "分区约束、ORC 约束、ACID 约束三大类完整覆盖"
  artifacts:
...
- `context/platform/hive-constraints.md` - Hive 3.x 平台约束（分区/ORC/ACID）
</objective>
...
文档包含：
- 约束速查表
- 至少 15 个编号约束（HIVE-001 ~ HIVE-016）
- P0 约束 >=8 个（关键约束）
- 每个约束有原因、规避方案
- P0/P1 约束有正/反代码示例
  </verify>
  <done>Hive 约束文档完整，覆盖分区/ORC/ACID/数据完整性四大类</done>
</task>
...
3. **Hive 约束文档：** hive-constraints.md 覆盖 PLATFORM-01 需求
4. **约束数量：** >=15 个编号约束，>=8 个 P0
5. **格式规范：** 遵循统一模板（ID/原因/后果/规避/示例）
6. **代码示例：** P0/P1 约束都有正/反示例
7. **Token 预算：** 索引页 <800, 约束文档 <2000
</verification>
