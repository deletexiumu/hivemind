...
must_haves:
  ...
  key_links:
    ...
    - from: ".claude/commands/dw/validate.md"
      to: "scripts/validate.js"
      via: "Bash 调用"
      pattern: "node.*validate\\.js"
    - from: ".claude/commands/dw/new-scenario.md"
      to: "scripts/scaffold.js"
      via: "Bash 调用"
      pattern: "node.*scaffold\\.js"
---
...
<task type="auto">
  ...
  <action>
**前置确认：** 确保 Plan 08-03 的脚本已创建且可运行（assemble.js, validate.js, scaffold.js）。
...
- 工具命令正确调用对应脚本
- 脚本依赖已安装（`cd .claude/data-warehouse && npm install` 成功）
</verification>
