...
**各场景输入 Schema 必填字段提取方法：**

1. **首选来源：** 各场景 prompt.md 的 frontmatter `required_input` 字段（如果存在）
2. **备选来源：** 从各场景 prompt.md 中的"必填最小集"/"输入规范"部分提取
3. **参考案例：** 查看 examples/ 目录下的示例文件，确认实际使用的输入结构

**各场景输入 Schema 必填字段（已确认）：**

1. **design-new-model:** business_event（业务事件）, grain（粒度）
   - 来源：prompt.md 中"Grain 声明"强制要求，examples/e-commerce-order.md 验证
2. **review-existing-model:** model_code（模型代码，SQL/dbt）
   - 来源：prompt.md 中"输入模型代码"为必需
3. **generate-sql:** requirement（取数需求）, source_tables（数据源表）
   - 来源：prompt.md 中明确需要需求和数据源
4. **define-metrics:** metric_name（指标名称）, business_desc（业务描述）
   - 来源：prompt.md 中指标定义需要名称和业务语义
5. **generate-dq-rules:** model_name（模型名称）, columns（字段清单）
   - 来源：prompt.md 中 DQ 规则需要目标模型和字段信息
6. **analyze-lineage:** target_model（目标模型）
   - 来源：prompt.md 中血缘分析需要目标模型
