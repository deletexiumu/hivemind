...
depends_on: []
files_modified:
  - .claude/data-warehouse/context/methodology/  # Task 0: 创建目录
  - .claude/data-warehouse/context/layers/       # Task 0: 创建目录
...
<task type="auto">
  <name>Task 0: 创建方法论库目录结构</name>
  <files>
    .claude/data-warehouse/context/methodology/
    .claude/data-warehouse/context/layers/
  </files>
  <action>
创建 Phase 02 所需的两个目录：

```bash
mkdir -p .claude/data-warehouse/context/methodology
mkdir -p .claude/data-warehouse/context/layers
```

这两个目录将分别存放：
- `methodology/` — METHOD-01/02/03 文档（维度建模、事实表、SCD）
- `layers/` — METHOD-04 文档（分层体系规范）

注意：这是并行 Wave 1 计划的共享前置步骤，确保 02-01/02/03 三个计划都能正常创建文件。
  </action>
  <verify>
    ```bash
  ...
  <done>methodology/ 和 layers/ 目录已创建</done>
</task>
...
2. **文档内容结构**：
   ...
   |------|---------|---------|-----------|------|
   | [dimensional-modeling.md](./dimensional-modeling.md) | METHOD-01 | 模式与一致性 | Kimball 四步法、星型/雪花、一致性维度、Bus Matrix、特殊维度模式 | 本计划创建 |
   | [fact-table-types.md](./fact-table-types.md) | METHOD-02 | 事实与口径 | 事务/周期快照/累积快照、可加性、查询模式 | Plan 02-02 创建 |
   | [scd-strategies.md](./scd-strategies.md) | METHOD-03 | 历史与回溯 | SCD Type 1/2/3 选型、dbt-hive 实现（无 Snapshots） | Plan 02-02 创建 |
   | [layering-system.md](../layers/layering-system.md) | METHOD-04 | 落层与治理边界 | ODS/DWD/DWS/ADS 定义、跨层规则 | Plan 02-03 创建 |

   > **注意**: 以上四个文档由 Wave 1 的三个并行计划分别创建。执行完成后所有链接将可用。
