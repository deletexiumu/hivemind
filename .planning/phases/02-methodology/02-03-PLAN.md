...
```bash
# 检查 methodology/ 目录下的 3 个文件
test -f .claude/data-warehouse/context/methodology/dimensional-modeling.md && echo "OK: dimensional-modeling.md" || echo "MISSING: dimensional-modeling.md"
test -f .claude/data-warehouse/context/methodology/fact-table-types.md && echo "OK: fact-table-types.md" || echo "MISSING: fact-table-types.md"
test -f .claude/data-warehouse/context/methodology/scd-strategies.md && echo "OK: scd-strategies.md" || echo "MISSING: scd-strategies.md"

# 检查 layers/ 目录下的 1 个文件
test -f .claude/data-warehouse/context/layers/layering-system.md && echo "OK: layering-system.md" || echo "MISSING: layering-system.md"

# 验证索引页中的链接语法正确
grep -E '\[.*\]\(\./.*\.md\)|\[.*\]\(\.\./layers/.*\.md\)' .claude/data-warehouse/context/methodology/index.md
```
