...
@~/.claude/hivemind/references/git-integration.md
</required_reading>
...
2. Use Task tool with subagent_type="dw-executor" and model="{executor_model}":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="dw-executor" and model="{executor_model}":
...
See `~/.claude/hivemind/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ~/.claude/hivemind/references/checkpoints.md for complete checkpoint guidance.
</step>
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `~/.claude/hivemind/templates/user-setup.md`.
...
Use ~/.claude/hivemind/templates/summary.md for structure.
...
**Also available:**
- `/dw:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
- `/dw:discuss-phase {Z+1}` — gather context first
...
**Also available:**
- `/dw:verify-work` — manual acceptance testing before completing milestone
- `/dw:add-phase <description>` — add another phase before completing
