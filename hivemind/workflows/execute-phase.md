...
**No dependency analysis needed.** Wave numbers are pre-computed during `/dw:plan-phase`.
...
2. **Read files and spawn all autonomous agents in wave simultaneously:**
   ...
   @~/.claude/hivemind/references/tdd.md
   </execution_context>
...
1. **Spawn agent for checkpoint plan:**
   ```
   Task(prompt="{subagent-task-prompt}", subagent_type="dw-executor", model="{executor_model}")
   ```
...
| `gaps_found` | Present gap summary, offer `/dw:plan-phase {phase} --gaps` |
...
- `/dw:verify-work {X}` — manual testing before planning
```

User runs `/dw:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `/dw:execute-phase {X} --gaps-only`
...
1. Run `/dw:execute-phase {phase}` again
