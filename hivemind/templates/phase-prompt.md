...
> **Note:** Planning methodology is in `agents/dw-planner.md`.
...
@~/.claude/hivemind/references/checkpoints.md
</execution_context>
...
<!-- For checkpoint task examples and patterns, see @~/.claude/hivemind/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/dw:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/hivemind/templates/user-setup.md` for full schema and examples
...
See `~/.claude/hivemind/workflows/verify-phase.md` for verification logic.
